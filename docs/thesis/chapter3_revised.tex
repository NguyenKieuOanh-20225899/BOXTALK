...
khối nội dung và đơn vị truy xuất. Các bộ dữ liệu như DocLayNet và PubLayNet cho thấy nhận diện
bố cục tài liệu là một bài toán quan trọng trong khai thác PDF~
\cite{pfitzmann2022doclaynet,zhong2019publaynet,shen2021layoutparser}.

Trong đồ án, mỗi vùng nội dung được biểu diễn bằng khung bao (bbox), loại nội dung
và siêu dữ liệu nguồn. Với PDF có lớp văn bản,
...
khối nội dung có trang, bbox, loại khối nội dung, thứ tự đọc và ngữ cảnh đề mục, làm nền cho
...
trúc, không chỉ là lấy chữ trong vùng bảng. Các khảo sát về nhận dạng bảng cũng xem bài toán này là quá trình suy luận cấu
trúc từ các dấu hiệu hình học và văn bản~\cite{zanibbi2004survey}.

Table Transformer được xây dựng trên hướng phát hiện đối tượng bằng Transformer,
kế thừa các ý tưởng từ Transformer và DETR~
...
\hline
Phát hiện vùng bảng &
Xác định khung bao bảng để cắt ảnh bảng hoặc lấy các từ nằm trong vùng bảng. &
...
Chia đoạn là bước chuyển khối nội dung thành đơn vị truy xuất. Trong đồ án, đơn vị truy
xuất không chỉ là một đoạn cắt theo độ dài, mà còn mang siêu dữ liệu như tài liệu
nguồn, trang, loại khối nội dung, đường dẫn đề mục, bbox và thông tin bảng nếu có. Các
...
Khi các vector đã được chuẩn hóa, điểm truy xuất ngữ nghĩa có thể tính bằng tích vô hướng:
...
điểm truy xuất kết hợp được tính bằng cách chuẩn hóa và cộng trọng số:
...
sắp xếp lại một tập ứng viên nhỏ. Phương pháp của Nogueira và Cho minh họa
hướng sắp xếp lại ứng viên bằng BERT~\cite{nogueira2019passage}. ColBERT sử
dụng cơ chế tương tác muộn giữa biểu diễn truy vấn và tài liệu~
\cite{khattab2020colbert}. Trong mã nguồn của đồ án, bộ sắp xếp lại có thể là
luật kinh nghiệm, cross-encoder hoặc ColBERT tùy cấu hình:
...
sẵn của mô hình ngôn ngữ. Vấn đề tỷ lệ thông tin không được nguồn hỗ trợ trong sinh ngôn ngữ tự nhiên và đánh giá RAG cho thấy câu
trả lời có vẻ hợp lý vẫn có thể không được nguồn hỗ trợ~
\cite{ji2023hallucination,es2023ragas}.
...
Tỷ lệ thành công cho biết quy trình có hoàn tất xử lý hay không, Token F1 và CER đo
...
SciFact và QASPER được dùng làm cơ sở dữ liệu tập đánh giá liên quan đến truy
...
Với \(N\) câu hỏi, các độ đo hỏi đáp trong đồ án được dùng ở dạng tổng quát:
...
Với phát hiện vùng bảng hoặc ô bảng, độ chồng khớp giữa khung bao dự đoán \(B_p\)
và khung bao nhãn chuẩn \(B_g\) được đo bằng IoU:
...
Từ các cặp dự đoán--nhãn chuẩn được ghép đúng theo ngưỡng IoU, hệ thống tính
...
Recall cho biết hệ thống tìm được bao nhiêu đối tượng đúng trong nhãn chuẩn,
còn F1 là trung bình điều hòa giữa hai độ đo này. Khi đánh giá phát hiện bảng,
một dự đoán chỉ được tính là đúng nếu khung bao dự đoán khớp với khung bao nhãn chuẩn
...
Ngoài Exact CSV, đồ án còn dùng thêm Cell F1, Structure F1 và Text Assignment F1
để phân tích chi tiết hơn. Với Cell F1, gọi \(C^{pred}\) và \(C^{gold}\) là tập
...
cột, quan hệ gộp ô hoặc quan hệ kề nhau giữa các ô. Gọi \(S^{pred}\) và \(S^{gold}\) là tập
...
\hline
Xử lý PDF &
Đọc lớp văn bản, lấy tọa độ, kết xuất trang và chuẩn hóa khối nội dung. &
